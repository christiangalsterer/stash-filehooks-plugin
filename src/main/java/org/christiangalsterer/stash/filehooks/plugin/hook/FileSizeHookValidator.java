...
public class FileSizeHookValidator implements SettingsValidator {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(FileSizeHookValidator.class);
        ...
        for (int i = 1; i <= patternParams; i++) {
           try {
            ...
            if (!Strings.isNullOrEmpty(settings.getString(SETTINGS_EXCLUDE_PATTERN_PREFIX + i))) {
                try {
                    Pattern.compile(Objects.requireNonNull(settings.getString(SETTINGS_EXCLUDE_PATTERN_PREFIX + i)));
                } catch (PatternSyntaxException e) {
            ...
            if (!Strings.isNullOrEmpty(settings.getString(SETTINGS_BRANCHES_PATTERN_PREFIX + i))) {
                try {
                    Pattern.compile(Objects.requireNonNull(settings.getString(SETTINGS_BRANCHES_PATTERN_PREFIX + i)));
                } catch (PatternSyntaxException e) {
